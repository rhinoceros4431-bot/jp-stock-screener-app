...
def api_chart(code):
    ...
    過去の類似パターン探索(pattern_match)のため2年分取得し、画面表示用のチャート自体は
    従来通り直近6か月程度に絞って返す。"""
    if not CODE_RE.match(code):
        return jsonify({"error": "invalid code"}), 400
    try:
        hist = fetch_single_quote(code, period="2y")
    except Exception as e:
    ...
    try:
        ...
        dates_full = hist["Date"].dt.strftime("%Y-%m-%d").tolist()
        pattern = pattern_match.find_similar_patterns(close_full, dates_full)
    except Exception as e:
        pattern = {"available": False, "reason": f"error: {e}"}

    # 画面表示用のチャートは直近130営業日(≒6か月)程度に絞る。類似パターン探索には
    # 上で取得した2年分(close_full)を使っているため、表示は絞っても問題ない。
    display = hist.tail(130).reset_index(drop=True)
    close = display["Close"].astype(float)
    ...
    return jsonify({
        "code": code,
        "dates": display["Date"].dt.strftime("%Y-%m-%d").tolist(),
        "close": _round_list(close),
        "volume": [int(v) for v in display["Volume"].fillna(0)],
        "ma5": _round_list(ma5),
